  ...
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-5">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-slate-500">{items.length ? `${items.length} находок` : "—"}</span>
      </div>
      ...
      <div className="mt-4 space-y-3">
        ...
        {!items.length && <div className="text-sm text-slate-600">Пока опасные формулировки не выделены.</div>}
      </div>
    </section>
  );
}
